...
- **Pre-filled fields** (title, year, venue in Reviewing; title, year in Checking): shown read-only, with a pencil button that makes the field editable; blur or Enter returns to display mode. In Checking, only Year is editable this way — Title stays read-only.
- **Checking source metadata** (from `check_papers` migration 9, source data — not reviewer-editable): `language` shows as a read-only field next to Title/Year, hidden if empty (hidden entirely for records that predate migration 9). `filters`/`filter_explanations` (automated screening-pipeline eligibility checks) are in the schema but not currently surfaced in the UI.
