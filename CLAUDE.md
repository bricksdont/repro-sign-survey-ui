...
- **Version badge**: a small muted `v<version>` link with a GitHub icon, bottom-center on every page (`index.html`, `review-index.html`, `paper.html`, `check-index.html`, `paper-check.html`, `stats.html`), pointing at the matching GitHub release tag — a quick visual check that you're not looking at a stale cached page, per issue #21 and a later follow-up extending it beyond just the landing page. `initVersionBadge()` in `api.js` is the single shared implementation (fetches `package.json` at runtime and reads `.version`; fails silently if unavailable) — each page calls it once from its own bootstrap/`init()`, and it's a no-op if the page's markup (`#version-badge`/`#version-badge-text`) isn't present. `package.json`'s `version` field is the single source of truth — bump it with `npm version <patch|minor|major>` when cutting a release, which atomically bumps the field, commits, and creates the matching `vX.Y.Z` git tag, so the badge and the tag can't drift apart. Datasets/Metrics catalogue pages don't have it yet.
