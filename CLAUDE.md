...
- **Checking source metadata** (from `check_papers` migration 9, source data — not reviewer-editable): `language` shows as a read-only field next to Title/Year, hidden if empty. `filters` (a JSON object of automated eligibility checks, e.g. `{"year": true, "area": false, ...}`) renders as pass/fail badges labelled from the object's keys; a badge's `filter_explanations` entry (if present for that key) is shown as its hover tooltip. Both the language field and the filter badges are hidden entirely for records that predate migration 9 (no `language`/`filters` data).
