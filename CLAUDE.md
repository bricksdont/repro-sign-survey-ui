...
- **Reviewing overview** (`review-index.html`): lists all papers from the `papers` collection with ID, title, status badge, and a Review link. Shows counts per status. Search box filters by ID or title (live, substring). Status filter pills narrow to a specific status. "Review Next →" navigates to a random `needs_review` paper that isn't currently locked by another user (issue #71) — `allPapers` already carries `locked_by`/`locked_at` from the initial fetch, so this needs no extra request; disabled with "No unlocked papers left to review" once every remaining `needs_review` paper is locked.
...
- **Filtered navigation subset** (issue #75): clicking a row on `review-index.html`/`check-index.html` records the currently filtered/searched paper IDs (in display order, across all pages, not just the visible page) into `sessionStorage` — `pb_review_nav_order` / `pb_check_nav_order` — overwritten on every row click, including unfiltered ones, so a stale entry from an earlier filter never lingers. `paper.html`/`paper-check.html` read this on load (`initNavOrder()`): if it's present and contains the paper being opened, ◀ ▶ and the `X / Y` counter stay within that subset; otherwise (direct/copied link, new tab, or a subset that doesn't include this paper) they fall back to the full collection. `sessionStorage` is same-tab and session-scoped by design — a copied or shared paper link intentionally loses the filter context and just navigates the full collection, so links stay simple and shareable (no filter state encoded in the URL). If the loaded paper ever falls outside the recorded subset (e.g. after Finalize & Next lands on a paper the subset didn't include — that flow is intentionally left as full-collection, unfiltered), navigation self-heals by resetting to the full collection rather than getting stuck.
- **Overview filter/search restoration** (issue #75 follow-up): `review-index.html`/`check-index.html` also persist the search box text and active status pill to `sessionStorage` (`pb_review_filter_state` / `pb_check_filter_state`) on every `applyFilters()` call, and restore + reapply them (`restoreFilterState()`) at the top of `init()`. This means using the paper detail page's `← Back` link — or any same-tab navigation back to the overview — lands on the same filtered view instead of always resetting to the unfiltered list. Since it fires on every load (not only "came back from a paper"), any same-tab visit to the overview page reuses the last filter from that session; a fresh tab or cleared `sessionStorage` starts unfiltered as before.
...
Playwright tests cover (38 tests total):
