...
let navQuery  = ''; // ?q= from the URL — mirrors check-index.html's search box
let navStatus = 'all'; // ?status= from the URL — mirrors check-index.html's status pill
...
async function init() {
  ...
  // ?status= (if present) carry over check-index.html's active search/filter
...
// check-index.html's applyFilters() uses, so the subset a paper was opened
...
function computeNavOrder() {
  ...
  navOrder = papers.filter(p => {
    ...
    const matchesStatus = navStatus === 'all' || (p.status || 'needs_check') === navStatus;
...
// check-index.html's buildFilterQuery()).
...
// Keeps the Back link pointed at check-index.html with the same filter
...
function updateBackLink() {
  ...
  document.querySelector('.back-link').href = `check-index.html${qs ? '?' + qs : ''}`;
}
...
async function loadPaper(index) {
  ...
  // subset (e.g. Save & Next intentionally landed outside it — that flow
  // always searches the full needs_check pool), fall back to the full
  ...
  document.title = 'REPRO-SIGN Survey Tool — Checking';
  updatePaperNav();
  updateBackLink();
