...
- Overview table with name, license, assignees (first one shown if there are several), availability badge, and URL; clicking a row opens the dataset detail page
- Search box plus six filters (Assigned to me, Availability, On Modal, Correspondence, orphaned/hide-orphaned, used in a Final paper) — all combine, "Showing X of Y" appears once active, and the active filters are reflected in the URL and preserved through a round trip to the dataset detail page and back
- "+ Add Dataset" navigates to a blank detail page; saving POSTs a new record and updates the URL to `?id=<id>`
- Detail page has fields for name, license, URLs (chip list), assignees, availability, whether it's stored on Modal.com, correspondence status with the data owners, and comments; edit locking with 30-minute heartbeat
- Assignees is self-assign-only: a single "Assign myself" / "Remove myself" toggle adds or removes your own email — no picker of other users
- ◀ ▶ buttons step through the same filtered/searched selection the dataset was opened from, with unsaved changes prompting to save before continuing
...
`server.py` also serves `GET /export?collection=<papers|datasets|reproductions>&id=<id>` — a curlable/scriptable JSON export. Requires an `Authorization: Bearer <token>` header (your own PocketBase token — the endpoint has no service credential of its own, so it can only ever see what your session already can); omit `id` to export the whole collection. It automatically picks a local vs. deployed PocketBase the same way the frontend does — hit it via `http://localhost:8765/export?...` and it uses your local backend, no env var needed. `id` means different things per collection, matching how each is already addressed elsewhere in the app: the `paper_id` slug for `papers` and (via its paper) for `reproductions`, and PocketBase's own internal record id for `datasets`. `locked_by`/`locked_at` are stripped from the response, including from `papers`' expanded `datasets`/`metrics`. See CLAUDE.md's "JSON export endpoint" section for the full details, including a PocketBase quirk worth knowing: an invalid/expired token isn't rejected with a `401` — it's silently treated as no auth at all, so you get an empty export rather than an error.

A plain link can't attach that header, so the Reviewing and Reproduction detail pages also have their own "Download JSON" button — the in-app way to get that one record as a file with one click, no `curl`/devtools needed. Datasets' detail page and every overview page don't have a button — they already show the full record/list on screen, and the underlying export is still available directly via `/export`.
