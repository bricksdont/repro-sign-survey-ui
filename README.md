...
A plain link can't attach that header, so each detail page (Reviewing, Datasets, Reproduction) also has its own "Download JSON" button — the in-app way to get that one record as a file with one click, no `curl`/devtools needed. Overview pages don't have a button — they already show the full list on screen, and the underlying full-collection export is still available directly via `/export`.
